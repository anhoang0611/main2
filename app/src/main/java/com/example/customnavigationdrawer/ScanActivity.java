...
//                }
    ...
    private void getValue(BluetoothDevice device, UUID uuid) {
        ...
        ref.child(DeviceID).child("quet").child(device.getAddress()).child("timestamp").setValue(ServerValue.TIMESTAMP);
        ref.child(DeviceID).child("quet").child(device.getAddress()).child("time").setValue(getDate(timestamp));
        ref.child(DeviceID).child("quet").child(device.getAddress()).child("name").setValue(device.getName().toString());
        i++;
    }
